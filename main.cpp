...
double michelson_contrast(cv::Mat image, std::vector<cv::Vec2i> inkpts, std::vector<cv::Vec2i> papypts) {
        double ink_avg = 0.0;  
...
double rms_contrast(cv::Mat image,box region) {
        cv::Mat subImg = image(cv::Range(region.ymin,region.ymax), cv::Range(region.xmin,region.xmax)).clone();
        ...
        double imageSize = (region.ymax - region.ymin) * (region.xmax - region.xmin);
...
int main(int argc, char** argv) {
        ...
        */
        ...
        for (auto paths : imgpaths)
        {
                ...
                double contrast1 = rms_contrast(image,inkPapy1);
                double contrast2 = rms_contrast(image,inkPapy2);
                double contrast3 = rms_contrast(image,inkPapy3);
